...
fn induce_sa_l<Char: Copy + Into<usize>>(t: &BitVec, sa: &mut [usize], s: &[Char], bkt: &mut [usize]) {
...
fn induce_sa_s<Char: Copy + Into<usize>>(t: &BitVec, sa: &mut [usize], s: &[Char], bkt: &mut [usize]) {
...
fn sais_inner<Char: Copy + Ord + Into<usize> + TryFrom<usize>>(s: &[Char], sa: &mut [usize], k: usize) {
    ...
    for i in 0..n1 {
        let pos = sa[i];
        for d in 0.. {
            if prev == usize::MAX || pos.max(prev) + d == s.len() || s[pos + d] != s[prev + d] || t[pos + d] != t[prev + d] {
                name += 1;
                prev = pos;
                break;
            }
...
proptest::proptest! {
    #[test]
    fn valid_bytes(s in proptest::string::bytes_regex(".*").unwrap()) {
        let sa = sais(&s);
        for neighbors in sa.windows(2) {
            proptest::prop_assert!(&s[neighbors[0]..] < &s[neighbors[1]..]);
        }
    }
